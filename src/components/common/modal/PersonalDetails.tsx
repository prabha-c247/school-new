...
const PersonalDetails: React.FC<ModalProps> = ({ modalOpen, Hide }) => { 
  ...
  const { profileImage, contactNumber, name } = useSelector((state: RootState) => state.setting);
    ...
    <Modal show={modalOpen} onHide={Hide}>
      ...
      <Row>          
          ...
          <div style={{ height: "100px", width: "100px" }}>
            <img src={profileImage || undefined} className="rounded mx-auto d-block img-thumbnail" alt="user profile"/>
          </div>
          </div>     
        </Row>
        <Row>
